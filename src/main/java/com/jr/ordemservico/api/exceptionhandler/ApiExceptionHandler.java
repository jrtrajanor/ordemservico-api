...
import com.jr.ordemservico.api.exceptionhandler.Problema.Campo;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> handleEntidadeNaoEncontrada(NegocioException ex, WebRequest request){
		...
		Problema problema = setProblema(status.value(), ex.getMessage(), null);
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatus status, WebRequest request){
		...
		Problema problema = setProblema(status.value(), ConstantesMessage.MSG_PREENCHIMENTO_CAMPOS_INCORRETOS, campos);		
		
		return super.handleExceptionInternal(ex, problema, headers, status, request);
	}
	
	private Problema setProblema(Integer status, String titulo, ArrayList<Campo> campos) {
